import { useState, useEffect } from 'react'

import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'
import { Switch } from '@/components/ui/switch'
import type { ConvertConfig } from '@/types/beets-config'
import PluginSettingsDialog from './PluginSettingsDialog'

interface ConvertSettingsDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  config: ConvertConfig
  onSave: (config: ConvertConfig) => void
}

export default function ConvertSettingsDialog({
  open,
  onOpenChange,
  config,
  onSave,
}: ConvertSettingsDialogProps) {
  const [localConfig, setLocalConfig] = useState<ConvertConfig>(config)

  // Reset local state when dialog opens
  useEffect(() => {
    if (open) {
      setLocalConfig(config)
    }
  }, [open, config])

  const handleSave = () => {
    onSave(localConfig)
  }

  const handleCancel = () => {
    setLocalConfig(config)
  }

  return (
    <PluginSettingsDialog
      open={open}
      onOpenChange={onOpenChange}
      pluginName="convert"
      onSave={handleSave}
      onCancel={handleCancel}
    >
      {/* Auto Convert Toggle */}
      <div className="flex items-center justify-between p-4 border rounded-md bg-muted/30">
        <div className="space-y-0.5">
          <Label htmlFor="convert-auto" className="text-base">
            Auto Convert on Import
          </Label>
          <p className="text-xs text-muted-foreground">
            Automatically convert files when importing to the library
          </p>
        </div>
        <Switch
          id="convert-auto"
          checked={localConfig.auto}
          onCheckedChange={(checked) =>
            setLocalConfig({ ...localConfig, auto: checked })
          }
        />
      </div>

      {/* FFmpeg Path */}
      <div className="space-y-2">
        <Label htmlFor="convert-ffmpeg">FFmpeg Path</Label>
        <p className="text-xs text-muted-foreground">Path to the ffmpeg binary</p>
        <Input
          id="convert-ffmpeg"
          value={localConfig.ffmpeg}
          onChange={(e) =>
            setLocalConfig({ ...localConfig, ffmpeg: e.target.value })
          }
          placeholder="/usr/bin/ffmpeg"
          className="font-mono text-sm"
        />
      </div>

      {/* FFmpeg Options */}
      <div className="space-y-2">
        <Label htmlFor="convert-opts">FFmpeg Output Options</Label>
        <p className="text-xs text-muted-foreground">
          Command-line options passed to ffmpeg for encoding
        </p>
        <Input
          id="convert-opts"
          value={localConfig.opts}
          onChange={(e) =>
            setLocalConfig({ ...localConfig, opts: e.target.value })
          }
          placeholder="-ab 320k -ac 2 -ar 48000"
          className="font-mono text-sm"
        />
        <p className="text-xs text-muted-foreground">
          Example: <code className="bg-muted px-1 rounded">-ab 320k</code> for
          320kbps bitrate
        </p>
      </div>

      {/* Max Bitrate */}
      <div className="space-y-2">
        <Label htmlFor="convert-max-bitrate">Max Bitrate (kbps)</Label>
        <p className="text-xs text-muted-foreground">
          Files already below this threshold will not be converted
        </p>
        <Input
          id="convert-max-bitrate"
          type="number"
          min={64}
          max={512}
          value={localConfig.max_bitrate}
          onChange={(e) =>
            setLocalConfig({
              ...localConfig,
              max_bitrate: parseInt(e.target.value) || 320,
            })
          }
          placeholder="320"
          className="w-32"
        />
      </div>

      {/* Threads */}
      <div className="space-y-2">
        <Label htmlFor="convert-threads">Conversion Threads</Label>
        <p className="text-xs text-muted-foreground">
          Number of parallel conversion threads (1 for single-threaded)
        </p>
        <Input
          id="convert-threads"
          type="number"
          min={1}
          max={16}
          value={localConfig.threads}
          onChange={(e) =>
            setLocalConfig({
              ...localConfig,
              threads: parseInt(e.target.value) || 1,
            })
          }
          placeholder="1"
          className="w-32"
        />
      </div>
    </PluginSettingsDialog>
  )
}
