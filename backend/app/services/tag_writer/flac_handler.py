"""FLAC/Vorbis comment tag handler implementation."""
...
from .mappings import FLAC_TAG_MAPPING, CanonicalTag

logger = logging.getLogger(__name__)

# Reverse mapping from Vorbis comment to canonical tag
VORBIS_TO_CANONICAL = {v: k for k, v in FLAC_TAG_MAPPING.items()}


class FLACTagHandler(TagHandler):
    ...
    def read_tags(self, file_path: str) -> Dict[str, Optional[str]]:
        ...
        tags: Dict[str, Optional[str]] = {tag: None for tag in FLAC_TAG_MAPPING.keys()}
        ...
        try:
            try:
                ...
                raise CorruptedFileError(
                    f"Invalid FLAC file (no header): {file_path}",
                    file_path=file_path,
                )
            ...
            logger.error(f"Error reading FLAC tags from {file_path}: {e}")
            raise CorruptedFileError(f"Failed to read FLAC tags: {e}", file_path=file_path)
        ...
        try:
            try:
                ...
                return TagWriteResult(
                    ...
                    file_path=file_path,
                    tags_written={},
                    error=f"Invalid FLAC file (no header): {file_path}",
            ...
            for canonical_tag, value in tags.items():
                vorbis_key = FLAC_TAG_MAPPING.get(canonical_tag)
                if not vorbis_key:
